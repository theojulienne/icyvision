/*****************************************************************************
 * vgltex.c: memory video driver for vlc
 *****************************************************************************
...
vlc_module_begin( );
    set_description( N_( "OpenGL Texture Output" ) );
    set_shortname( N_("OpenGL Texture") );

    set_category( CAT_VIDEO );
    set_subcategory( SUBCAT_VIDEO_VOUT );
    set_capability( "video output", 12 );

    add_integer( "vgltex-width", 320, NULL, T_WIDTH, LT_WIDTH, false );
    add_integer( "vgltex-height", 200, NULL, T_HEIGHT, LT_HEIGHT, false );
    add_integer( "vgltex-pitch", 640, NULL, T_PITCH, LT_PITCH, false );
    add_string( "vgltex-chroma", "RV16", NULL, T_CHROMA, LT_CHROMA, true );
    add_string( "vgltex-lock", "0", NULL, T_LOCK, LT_LOCK, true );
    add_string( "vgltex-unlock", "0", NULL, T_UNLOCK, LT_UNLOCK, true );
    add_string( "vgltex-data", "0", NULL, T_DATA, LT_DATA, true );
...
static int Init( vout_thread_t *p_vout )
{
    ...
    i_width = config_GetInt( p_vout, "vgltex-width" );
    i_height = config_GetInt( p_vout, "vgltex-height" );
    i_pitch = config_GetInt( p_vout, "vgltex-pitch" );

    psz_chroma = config_GetPsz( p_vout, "vgltex-chroma" );
    if( psz_chroma )
    {
        if( strlen( psz_chroma ) < 4 )
        {
            msg_Err( p_vout, "vgltex-chroma should be 4 characters long" );
            free( psz_chroma );
            return VLC_EGENERIC;
        }
    ...
    psz_tmp = config_GetPsz( p_vout, "vgltex-lock" );
    p_vout->p_sys->pf_lock = (void * (*) (void *))(intptr_t)atoll( psz_tmp );
    free( psz_tmp );

    psz_tmp = config_GetPsz( p_vout, "vgltex-unlock" );
    p_vout->p_sys->pf_unlock = (void (*) (void *))(intptr_t)atoll( psz_tmp );
    free( psz_tmp );

    psz_tmp = config_GetPsz( p_vout, "vgltex-data" );
